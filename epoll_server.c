...
void epoll_run(int port)
{
    ...
    while(1) {
        ...
        /* 遍历发生变化的节点，特别注意，这里没有采用多进程或多线程的方法
        来处理多个客户端，而是在for循环里面对客户端的请求一个个处理完了在处理
        下一个客户端的请求！因此效率不高，没有采用多进程或多线程提高并发量，最好的是用
        线程池*/
        for(i=0; i<ret; ++i)
        {
            // 只处理读事件, 其他事件默认不处理
...
{
    // 拆分http请求行
    char method[12], path[128], protocol[12];
